...
import { Search, Flame, Trophy, Clock, Target, Award, ChevronDown, ChevronRight, ExternalLink, Calendar, BookOpen, Play, FileText, HelpCircle } from 'lucide-react';
import SageMascot from './SageMascot';
import QuizComponent from './QuizComponent'; // New import for QuizComponent

export default function Dashboard({
    profile,
    curriculum,
    quizzes, // New prop for quizzes
    generateCurriculum,
    generateQuiz, // New prop for generating quizzes
    searching,
    sageState,
    setShowCelebration,
    error,
    ...
    const handleSearch = (e: React.KeyboardEvent) => {
        if (e.key === 'Enter' && searchQuery.trim()) {
            ...
            generateQuiz(searchQuery); // Also generate a quiz when a curriculum is searched
        }
    };
    ...
    const toggleLessonComplete = (lessonId: string) => {
        ...
        if (newCompleted.has(lessonId)) {
            ...
            if (Math.random() > 0.7) {
                ...
                setTimeout(() => setShowCelebration(false), 2000); // Hide celebration after 2 seconds
            }
        }
        setCompletedLessons(newCompleted);
    };

    // Placeholder for uploaded resources - in a real app, this would come from a backend or state
    const uploadedResources = [
        { id: '1', title: 'The Lean Startup by Eric Ries', type: 'book', format: 'PDF', url: '#', description: 'A detailed guide on lean methodologies for startups.' },
        { id: '2', title: 'Machine Learning Cheatsheet', type: 'paper', format: 'PDF', url: '#', description: 'Quick reference for common ML algorithms and concepts.' },
        { id: '3', title: 'Advanced React Patterns', type: 'blog', format: 'Web Article', url: 'https://reactpatterns.com/', description: 'Explore various design patterns in React applications.' },
    ];
            ...
            <main className="max-w-6xl mx-auto p-6">
                ...
                {curriculum && (
                    <div className="space-y-8">
                        ...
                        {/* Quizzes Section */}
                        <div className="space-y-6">
                            <h2 className="text-3xl font-bold text-gray-800 mb-4">🧠 Quizzes</h2>
                            {quizzes.length === 0 && (
                                <p className="text-gray-600">No quizzes available yet. Generate a curriculum to get a quiz!</p>
                            )}
                        ...
                        {/* Aeron Academy Resources Section */}
                        <div className="space-y-6">
                            <h2 className="text-3xl font-bold text-gray-800 mb-4">📚 Your Uploaded Resources</h2>
                            {uploadedResources.length === 0 && (
                                <p className="text-gray-600">No resources uploaded yet. Upload books, papers, or other materials to get interactive courses or quizzes!</p>
                            )}
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {uploadedResources.map((resource) => (
                                    <div key={resource.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-2">
                                        <div className="flex items-center space-x-2 text-purple-600">
                                            {resource.type === 'book' && <BookOpen className="w-5 h-5" />}
                                            {resource.type === 'paper' && <FileText className="w-5 h-5" />}
                                            {resource.type === 'blog' && <ExternalLink className="w-5 h-5" />}
                                        ...
                                        <div className="flex justify-between items-center text-sm text-gray-500">
                                            ...
                                            {resource.url && (
                                                <a href={resource.url} target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:text-purple-800 flex items-center space-x-1">
